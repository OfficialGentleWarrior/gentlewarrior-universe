...
endRunBtn.addEventListener("click", async () => {
  isRunActive = false;   // stop the run
  isResolving = true;   // freeze board

  saveGame();           // local save
  await submitRunToLeaderboard(); // send to Firestore
  await loadLeaderboard();        // refresh Top 10 UI

  showEndRunOverlay(); // show Run Complete popup
});
